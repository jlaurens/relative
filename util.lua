...
BUILD = tonumber(execute_cmd('git rev-list --count HEAD'))
...
local function clean_source()
  file_remove('relative')
  file_remove('relative-doc')
  file_remove('relative-code')
end
...
elseif arg[1] == 'pkginfo' then
  local f = io.open('support/pkginfo.sty', 'w')
  f:write(string.format([[%% Generated by `texlua util.lua %s` on %s
\newcommand\RLTVYEAR{%d}
\newcommand\RLTVMINOR{%d}
\newcommand\RLTVBUILD{%s}
]], table.concat(arg, ' '), os.date("!%c"), YEAR, VERSION, BUILD))
  f:close()
elseif arg[1] == 'uploadconfig' then
  local lines = {}
  for line in io.lines('uploadconfig.lua') do
    lines[#lines+1] = line
  end
  local f = io.open('uploadconfig.lua', 'w')
  for _, line in pairs(lines) do
    if line:match('%s*version%s*=.*') then
      f:write(string.format([[  version = "%d.%d(%d)",
]], YEAR, VERSION, BUILD))
    else 
      f:write(line..'\n')
    end
